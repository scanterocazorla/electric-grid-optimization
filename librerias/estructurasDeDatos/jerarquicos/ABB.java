...
public class ABB<E extends Comparable<E>> {
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        ...
        NodoABB<E> nodo= new NodoABB<E>(v[mitad],construirEquilibrado(v,ini, mitad-1),construirEquilibrado(v,mitad+1,fin));
        nodo.talla = 1;
        if(nodo.izq != null){
            nodo.talla =  nodo.talla + nodo.izq.talla;
        }
        if(nodo.der != null){
            nodo.talla = nodo.talla + nodo.der.talla;
        }
        return nodo;
    }
    ...
    public void reconstruirEquilibrado() {        
        E[] res=toArrayInOrden();
        raiz=construirEquilibrado(res, 0, res.length -1);
    }
