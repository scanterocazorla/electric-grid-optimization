...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        for (int i = 0; i<aristas.length; i++) {
            Arista a = aristas[i];
            Municipio origen = verticesAMunicipios.recuperar(a.getOrigen());
            Municipio destino = verticesAMunicipios.recuperar(a.getDestino());
            ListaConPI<Municipio> municipiosAdyacentes = adyKruskal.recuperar(origen);
            if (municipiosAdyacentes == null) {
                municipiosAdyacentes = new LEGListaConPI<Municipio>();
            }
            municipiosAdyacentes.insertar(destino);
            adyKruskal.insertar(origen, municipiosAdyacentes);
